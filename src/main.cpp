...
const int timeZone = -3 * 3600; // UTC-3. In seconds
const int daylightOffset = 0; // No daylight saving time. In seconds
...
void setup()
{
    ...
    WiFi.softAPsetHostname("enerlab");
    ...
    // Generate MQTT topic with MAC address
    String mac = WiFi.macAddress();
    mac.replace(":", "");
    mqtt_topic_str = String(MQTT_TOPIC_PREFIX) + mac;
    
    LOG_INFO(("MQTT Topic: " + mqtt_topic_str).c_str());

    configTime(timeZone, daylightOffset, ntpServer1, ntpServer2, ntpServer3);

    // Initialize UART1 for sensor communication
    uartSerial.begin(UART_BAUD, SERIAL_8N1, RXD1, TXD1);
    LOG_INFO("UART initialized on RX=%d, TX=%d, Baud=%d", RXD1, TXD1, UART_BAUD);
...
void loop()
{
    ...
    if (uartSerial.available())
    {
        ...
        if (uartData.length() > 0)
        {
            ...
            {
                // Update stored STM32 firmware version if provided, but do not forward it
                const char* stmVer = doc["version"].is<const char*>() ? doc["version"].as<const char*>() : nullptr;
                if (stmVer != nullptr)
                {
                    if (strlen(stmVer) > 0)
                    {
                        wm.setSTM32FirmwareVersion(stmVer);
                        LOG_INFO("STM32 firmware version updated to %s", stmVer);
                    }
                    else
                    {
                        LOG_DEBUG("'version' key present but empty");
                    }
                    doc.remove("version");
                }

                // Serialize sanitized JSON and publish to MQTT
                String jsonOutput;
                serializeJson(doc, jsonOutput);
                LOG_DEBUG((String("Publishing JSON (without version): ") + jsonOutput).c_str());
                
                if (mqttClient.connected())
                {
                    if (mqttClient.publish(mqtt_topic_str.c_str(), jsonOutput.c_str()))
                    {
